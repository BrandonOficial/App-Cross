...
export function CreateRoutineModal({ onClose, onSubmit, isLoading, exercises }: CreateRoutineModalProps) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-end justify-center" onClick={onClose}>
            ...
            {/* Bottom Sheet */}
            <div
                className="relative z-10 w-full max-w-[430px] bg-[#1a1a1a] rounded-t-3xl border-t border-white/10 p-6 pb-8 animate-in h-[85vh] flex flex-col"
                onClick={(e) => e.stopPropagation()}
            >
                {/* Handle */}
                <div className="w-10 h-1 bg-white/20 rounded-full mx-auto flex-shrink-0 mb-5" />
                ...
                {/* Title */}
                <h2 className="text-xl font-bold mb-1 flex-shrink-0">Nova Rotina</h2>
                <p className="text-xs text-muted-foreground mb-6 flex-shrink-0">
                    Crie um treino combinando exercícios do seu arsenal.
                </p>

                {/* Name Input */}
                <label className="block mb-6 flex-shrink-0">
                ...
                {/* Exercise List */}
                <div className="flex-1 overflow-y-auto min-h-0 -mx-2 px-2 mb-6">
                ...
                <button
                    ...
                    disabled={!canSubmit}
                    className={`flex-shrink-0 w-full py-3.5 rounded-xl font-bold text-sm uppercase tracking-wider transition-all ${
                        canSubmit
